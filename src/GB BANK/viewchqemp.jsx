...
class ViewCheques extends Component { 
...
ages:[25,30,35,40,45,50]};
  ...
  let response = await http.get(`/getChequeByName/${user.name}`);
...
if (prevProps!==this.props)this.fetchData();}
 callURL= (url, options) => {
  ...
  handlePage = (incr) => {
    ...
    this.callURL(`/viewCheques`, queryParams);
    };
    ...
    makeSearchString = (options) => {
      ...
      handleOptionChange = (options) => { 
        ...
        this.callURL("/viewCheques", options);
    };
render() {
 ...
 console.log(startindex)
  let EndIndex=Employees.length<max+startindex?Employees.length:max+startindex
